...
The install script symlinks skills into `~/.claude/commands/` (global) and `~/aic-vault/.claude/commands/` (vault). Folder-based skills in `dev/` are symlinked to `~/.claude/skills/`. It's idempotent and won't touch existing files that aren't managed by this repo.
...
- **`/defuddle`** — Defuddle tool reference
- **`/json-canvas`** — JSON Canvas spec reference

### Dev (development context, global availability)

- **`/obsidian-plugin-dev`** — Obsidian plugin development workflow
- **`raycast-extension-dev`** — Raycast extension development (folder skill with references)

## Adding a New Skill

1. Create a markdown file in `global/`, `vault/`, or `dev/` depending on scope.
